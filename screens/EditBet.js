...
export default function EditBet({ route, navigation }) {
  const { uid, header, amountWagered, profitLoss, sport } = route.params;
  const userID = auth.currentUser.uid;
  const [title, setTitle] = useState(header);
  const [wagered, setWagered] = useState(amountWagered);
  const [winnings, setWinnings] = useState(profitLoss);
  ...
  return (
    <KeyboardAvoidingView style={styles.container}>
      <View style={styles.inputContainer}>
        ...
        />
        <Text style={styles.input}>{`${ROI}%`}</Text>
        <Picker
